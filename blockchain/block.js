const { GENESIS_DATA, MINE_RATE }  = require('../config/config.js');
...
class Block{
    ...
    static adjustedDifficulty({ lastBlock, timestamp }){
    ...
    static minedBlock({ lastBlock, beneficiary, transactionSeries, stateRoot }){
    ...
    static validateBlock({ lastBlock, block, state }){
           ...
           return new Promise((resolve, reject) => {
              ...
              if(keccakHash(block) === keccakHash(Block.genesis())){
               ...
               if(rebuiltTransactionTrie.rootHash !== block.blockHeaders.transactionsRoot ){
                   return reject(  new Error(`The rebuilt transaction root does not match the block's transaction root: ${block.blockHeaders.transactionsRoot} `))
               }
